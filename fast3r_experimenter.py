#!/usr/bin/env python3
"""
fast3r_experimenter.py   •   17-May-2025
----------------------------------------
...
def run_timed(func, *args, **kwargs):
    """Run func(*args, **kwargs), return (output, elapsed_seconds)."""
...
def save_ply(xyz: np.ndarray, rgb: np.ndarray, path: Path):
    """Write coloured point cloud to compressed PLY."""
...
def process_scene(img_dir: Path, ws_dir: Path, size: int, device, dtype):
    ...
    # 2) Gather image files & read original RGBs
    ...
    # 7) Fuse only well‐masked, aligned points + original colours
    t_fuse = time.perf_counter()
    xyz_list, rgb_list = [], []
    for orig, pred in zip(orig_imgs, out["preds"]):
        # pick the aligned points if available
        if "local_pts3d_global_est" in pred:
...
def main():
    ...
    g.add_argument("--images",       help="folder with your scene images")
    g.add_argument("--dataset_root", help="batch: parent folder of scenes")
    ap.add_argument("--workspace", required=True,
                    help="where to write per-scene subdirs")
    ap.add_argument("--size",    type=int, default=512,
                    help="resize longer edge to this (default 512)")
    ...
    if args.images:
        process_scene(Path(args.images).expanduser().resolve(),
                      ws_root, size=args.size,
                      device=device, dtype=dtype)
    else:
        for scene in sorted(Path(args.dataset_root).expanduser().iterdir()):
            if scene.is_dir():
                process_scene(scene, ws_root/scene.name,
                              size=args.size,
                              device=device, dtype=dtype)
