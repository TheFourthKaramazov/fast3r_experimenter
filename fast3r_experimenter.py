...
def process_scene(img_dir: Path, ws_dir: Path, size: int, device, dtype,
                  pointmap_head: str, conf_threshold: float):
    ...
    # 7) Fuse + per-point confidence filter
    t_fuse = time.perf_counter()
    xyz_list, rgb_list = [], []
    for orig, pred in zip(orig_imgs, out["preds"]):
        # pick local‐head‐aligned or global‐head (in_other_view) points
...
def main():
    ...
    ap.add_argument("--workspace",    required=True, help="where to write outputs")
    ap.add_argument("--size",         type=int, default=512, help="resize longer edge")
    ap.add_argument("--dtype",        choices=("fp32","bf16"), default="fp32")
    ap.add_argument("--pointmap_head",choices=("local","global"),default="local",
                    help="which point‐map to fuse")
    ...
    if args.images:
        process_scene(Path(args.images), ws,
                      size=args.size,
        ...
        for scene in sorted(Path(args.dataset_root).iterdir()):
            if scene.is_dir():
                process_scene(scene, ws/scene.name,
                              size=args.size,
                              device=device, dtype=dtype,
                              pointmap_head=args.pointmap_head,
                              conf_threshold=args.conf_threshold)
